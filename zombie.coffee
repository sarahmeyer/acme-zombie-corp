# Description:
#   This is a zombie. It's dead, but it keeps walking. It wants
#   to eat.
#
# Commands:
#   attack? - prints out the count of the number of times somebody said "up"
...
#
# Author:
#   Sarah Meyer, with a heavy debt to Michi Kono
#
module.exports = (zombie) ->
  ...
  zombie.respond /sleep and dream of brains/i, (msg) ->
  ...
  # counts every time somebody says "the zombie attacks"
  # prints out the count when somebody says "how many times did it hurt you?"
  ###
  ...
  zombie.hear /\the zombie attacks\b/, (msg) ->
    # note that this variable is *GLOBAL TO ALL SCRIPTS* so choose a unique name
    zombie.brain.set('times_attacked_baby', (zombie.brain.get('times_attacked_baby') || 0) + 1)

  # ? is a special character in regex so it needs to be escaped with a \
  # the i on the end means "case *insensitive*"
  zombie.hear /how many times did it hurt you\?/i, (msg) ->
    msg.send "the baby has been bitten by the zombie " + (zombie.brain.get('times_attacked_baby') || "0")
  ...
  zombie.router.get '/hubot/my-custom-url/:room', (req, res) ->
    zombie.emit "bug-me", {
  ...
  ###
  zombie.on "bug-me", (data) ->
    try
      # this will do a private message if the "data.room" variable is the user id of a person
      zombie.messageRoom data.room, 'This is a custom message due to ' + data.source
    catch error
